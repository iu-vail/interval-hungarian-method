...
Hungarian::HungarianAlgo(BipartiteGraph& _bg, vector<VID>& _S, vector<VID>& _T,
  ...
  while(cnt1++ < MaxLoop1){
    ...
    //iterate Hungarian procedure    
    this->HungarianIteration(_bg, _S, _T, _N, _EG, _M, plot);
...
//below code has been modified with args in overloaded function
//this piece directly operates on data member and is convenient monitor data
...
/*
  ...
  while(cnt1++ < MAX_LOOP){
	...
	if(!bg.GetTask(y)->GetMatched()){
	  vector<EID> _path = BFSAugmentingPath(u, y);
	  AugmentPath(_path);
	  RefreshBG();
  ...
  cout<<endl<<"@_@ The matched edges are:"<<endl;
  DisplayData(M);
  cout<<endl;
*/
...
double 
Hungarian::HungarianIteration(BipartiteGraph& _bg, vector<VID>& _S, vector<VID>& _T,
                        vector<VID>& _N, vector<EID>& _EG, vector<EID>& _M, 
			PlotGraph& plot, bool PlotMode){
...
bool 
Hungarian::IsPerfect(BipartiteGraph& _bg){

  unsigned int num_vertex = 0;
  //assume the number of agents and tasks are identical
  //check number of active agents
  for(unsigned int i=0; i<_bg.GetNumAgents(); i++)
    if(_bg.GetAgent(i)->GetActive())
      num_vertex++;

  bool result = _bg.GetNumMatched() == num_vertex ? true: false;  
...
bool 
Hungarian::IsPerfect(void){
 
  return this->IsPerfect(this->bg);
...
Hungarian::RefreshBG(BipartiteGraph& _bg, 
  ...
  if(!_bg.CheckFeasibility())
    cerr<<"Warning: I detected that bipartite graph is NOT feasible! Continue."<<endl;

  //check admissible edges, update admissible flags for edges, and update set EG
  _EG.clear();
  for(unsigned int i=0; i<_bg.GetNumAgents(); i++)
    for(unsigned int j=0; j<_bg.GetNumTasks(); j++){
      if(_bg.GetAgent(i)->GetActive() && _bg.GetTask(j)->GetActive() && 
		//check admissible edges, with small tolerence for double calculation
		fabs(_bg.GetMatrix(i,j)->GetWeight() -( _bg.GetAgent(i)->GetLabel() + _bg.GetTask(j)->GetLabel())) <= DOUBLE_EPSILON){
  ...
  for(unsigned int i=0; i<_bg.GetNumAgents(); i++)
    for(unsigned int j=0; j<_bg.GetNumTasks(); j++)
      if(_bg.GetMatrix(i,j)->GetMatchedFlag() &&
		_bg.GetAgent(i)->GetActive() && _bg.GetTask(j)->GetActive() ){
...
Hungarian::ExtendTree(BipartiteGraph& _bg, VID x, vector<VID>& _S, vector<VID>& _T){
  ...
  if(_bg.GetTask(x)->GetMatched()){
    for(unsigned int i=0; i<_bg.GetNumAgents(); i++)
      if(_bg.GetMatrix(i, x)->GetMatchedFlag() && _bg.GetAgent(i)->GetActive()){
	vid_agent = i;
	break;
      }
...
Hungarian::BFSAugmentingPath(BipartiteGraph& _bg, VID x, VID y){
  ...
  while(dq.size() && !found){
    ...
    if(v.GetObj() == "TASK"){
      if(_Verbose)
        cout<<"I am at "<<v.GetObj()<<" VID: "<<v.GetVID()<<endl;
      ...
      for(agent_index=0; agent_index<agents_size; agent_index++)
		...
		!_bg.GetAgent(agent_index)->path.size() && _bg.GetAgent(agent_index)->GetActive()){
	  if(_Verbose)
	...
	if(!_bg.GetMatrix(v.GetVID(), task_index)->GetMatchedFlag()
		...
		&& !_bg.GetTask(task_index)->path.size() && _bg.GetTask(task_index)->GetActive()){
	  if(_Verbose){
	    if(task_index == y)
	      cout<<"--Found target TASK VID: "<<task_index<<", augmenting path found!"<<endl;
	    else    //searching in EG, so task must be either a matched vertex or the target vertex
      ...
      //wierd situation
      cerr<<"Error: Some vertices are found not initialized with Object name. Stopped."<<endl;
      exit(-1);
    }
...
Hungarian::PickFreeAgent(BipartiteGraph& _bg){
  ...
  for(size_t i=0; i<_bg.GetNumAgents(); i++)
    if(!_bg.GetAgent(i)->GetMatched() && _bg.GetAgent(i)->GetActive()){
      free_agent = i;
      break;
    }
...
Hungarian::PickAvailableTask(vector<VID>& _T, vector<VID>& _N){
  ...
  for(vector<VID>::iterator itr1 = _N.begin(); itr1 != _N.end(); itr1++){
    ...
    //check if in T or not, if in T, break and check next one in N
    for(itr2 = _T.begin(); itr2 != _T.end(); itr2++)
      if(*itr2 == *itr1) break;
    //if not in T, then pick it and terminate loop
    if(itr2 == _T.end()){
      y=*itr1;
      break;
    }
...
Hungarian::UpdateLabels(BipartiteGraph& _bg){
  double delta;
  double _min_delta = (double)POS_INF;
  
  int ti=-1, tj=-1;
  //calculate the mininal delta for re-labelling
  for(size_t i=0; i<_bg.GetNumAgents(); i++){
    if(_bg.GetAgent(i)->GetColored() && _bg.GetAgent(i)->GetActive())        //agent is in set of S
      for(size_t j=0; j<_bg.GetNumTasks(); j++){
	if(!_bg.GetTask(j)->GetColored() && _bg.GetTask(j)->GetActive()){   //task is NOT in set of T
  ...
  if(_Verbose)
    cout<<"("<<ti<<","<<tj<<")->min_data: "<<_min_delta<<endl;
  ...
  for(size_t i=0; i<_bg.GetNumAgents(); i++)
    ...
    if(_bg.GetAgent(i)->GetColored() && _bg.GetAgent(i)->GetActive()){
  ...
  for(size_t j=0; j<_bg.GetNumTasks(); j++)
    //task is in set of T
    if(_bg.GetTask(j)->GetColored() && _bg.GetTask(j)->GetActive()){
...
vector<EID> 
Hungarian::GetUnMatchedSet(BipartiteGraph& _bg, vector<EID>& _M){

  vector<EID> set;
  for(unsigned int i=0; i<_bg.GetNumAgents(); i++)
    for(unsigned int j=0; j<_bg.GetNumTasks(); j++)
      if(_bg.GetAgent(i)->GetActive() && _bg.GetTask(j)->GetActive() &&
		!_bg.GetMatrix(i,j)->GetMatchedFlag())
	set.push_back(EID(i,j));
...
vector<EID> 
Hungarian::GetUnMatchedSet(void){
 
  return GetUnMatchedSet(this->bg, this->M);

}


void
Hungarian::DisplayData(const vector<VID>& v){
  for(vector<VID>::const_iterator itr = v.begin(); itr != v.end(); itr++)
    cout<<*itr<<" ";
  cout<<endl;
}

void
Hungarian::DisplayData(const vector<EID>& e){
  for(vector<EID>::const_iterator itr = e.begin(); itr != e.end(); itr++)
...
Hungarian::DisplayData(bool s, bool t, bool n, bool eg, bool m){
  ...
  if(s){
    cout<<"Set S:"<<endl;
    this->DisplayData(this->S); 
  }

  //displaying T
  if(t){
    cout<<"Set T:"<<endl;
    this->DisplayData(this->T);
  }

  //displaying N
  if(n){
    cout<<"Set N:"<<endl;
    this->DisplayData(this->N);
  }

  //displaying EG
  if(eg){
    cout<<"Set EG:"<<endl;
    this->DisplayData(this->EG);
  }

  //displaying M
  if(m){
    cout<<"Set M:"<<endl;
    this->DisplayData(this->M);
  }
...
void
Hungarian::DisplayLabels(const vector<Vertex>& vertices){
  for(vector<Vertex>::const_iterator itr = vertices.begin(); itr != vertices.end(); itr++)
    cout<<(*itr).GetLabel()<<" ";
  cout<<endl;
}
...
void 
Hungarian::DisplayMatrix(const Matrix& _m){
